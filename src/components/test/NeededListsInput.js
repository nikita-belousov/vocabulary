...
let NeededListsInput = class extends Component {
  ...
  componentWillMount() {
    this.props.allLists.forEach(list =>
      this.setState(({ data }) => ({
        data: data.set(list.get('id'), false)
      }))
    )
  }
  ...
  render() {
    ...
    const checkboxOptions = this.listsToCheckboxOptions(filteredLists)
